...
object Year2025Day07 : AdventOfCodeDaySolution {
    ...
    override fun computePart2(input: String): Long {
        val table: Table<DiagramElement> = Table(input) { DiagramElement.fromChar(it) }
        val numberOfPaths = MutableList(table.width()) { 0L }
        val startPosition = table.findValue(DiagramElement.START).single()
        numberOfPaths[startPosition.x] = 1

        for (y in 1 until table.height()) {
            for (x in 1 until table.width()) {
                if (table.valueAt(Position(x, y)) == DiagramElement.SPLITTER) {
                    numberOfPaths[x - 1] = numberOfPaths[x - 1] + numberOfPaths[x]
                    numberOfPaths[x + 1] = numberOfPaths[x + 1] + numberOfPaths[x]
                    numberOfPaths[x] = 0
                }
            }
